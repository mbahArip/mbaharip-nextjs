...
const Works = () => {
	...
	let itemsPerPage;
	isMobile ? (itemsPerPage = 5) : (itemsPerPage = 8);

	useEffect(() => {
		async function fetchData() {
			let d = await firestore.readCollection('posts', 'idIndex');
			d = d.filter((post) => {
				return post.category.toLowerCase() == 'works';
			});
			setData(d);
			...
			<main className='flex flex-col flex-1 w-full text-mbaharip-light'>
				...
				<div className='w-full md:w-screen-lg max-w-screen-lg mx-auto flex flex-col justify-between'>
					...
					<div className='w-full my-8 flex flex-col items-center justify-center'>
						<span className='font-mono font-bold text-4xl  tracking-wide md:tracking-widest mb-8'>My Works</span>
						<div className='mb-8'>
							<form
					...
					{currentItems ? (
						<>
							{/* Posts List */}
							<div className='grid grid-cols-1 md:grid-cols-4 gap-6 mx-4 my-8'>
								{currentItems.map((d, index) => (
									<PostCard Data={d} key={index} />
								))}
							</div>
							{/* Pagination */}
